...
export function AddOrderModal({
    ...
    const [items, setItems] = useState<
        { itemId: number; quantity: number; notes?: string }[] // Added notes field
    >([]);
    ...
    const addItem = () => {
        setItems([...items, { itemId: 0, quantity: 1, notes: "" }]); // Added notes
    };

    const updateItem = (
        index: number,
        field: "itemId" | "quantity" | "notes", // Added notes to field types
        value: number | string, // Allow string for notes
    ) => {
        const copy = [...items];
                                        ...
                                        <div className="grid grid-cols-1 gap-3 md:grid-cols-12 md:items-start">
                                            ...
                                            <div className="md:col-span-2">
                                                ...
                                                <div className="h-12 rounded-md bg-gray-50 px-3 py-2 text-right text-sm font-medium text-gray-700">
                                                    {item.itemId && item.itemId !== 0
                                                        ? `${unitPrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} VNĐ`
                                                        : "--"}
                                                </div>
                                            </div>
                                            ...
                                            <div className="md:col-span-2">
                                                ...
                                                <div className="h-12 rounded-md bg-blue-50 px-3 py-2 text-right text-sm font-bold text-blue-700">
                                                    {item.itemId && item.itemId !== 0
                                                        ? `${subtotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} VNĐ`
                                                        : "0.00"}
                                                </div>
                                            </div>
                ...
                <div className="rounded-lg border-2 border-blue-200 bg-blue-50 p-4">
                    <div className="flex items-center justify-between">
                        ...
                        <span className="text-2xl font-bold text-blue-600">
                            {totalAmount.toLocaleString('vi-VN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} VNĐ
                        </span>
                    </div>
                </div>
